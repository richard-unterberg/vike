...
export const registerScrollToPlugin = () => {
  if (scrollToPluginRegistered || typeof window === 'undefined') {
    return
  }
...
export const smoothScrollToTarget = (sectionNode: Element, offset = 0) => {
  ...
  gsap.to(window, {
    ...
    scrollTo: {
      ...
      // we don't user device interfer with the scroll (setting it to true is buggy on mobile)
      autoKill: false,
    },
  })
}
